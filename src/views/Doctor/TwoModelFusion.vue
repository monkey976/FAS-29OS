...
export default {
  ...
  setup() {
    onMounted(() => {
      // create rendering pipeline and load stl file
      const createVTKPipeline = (containerId: string, stlPath: string, opacityValue: int) => {
        ...
        vtkHttpDataAccessHelper.fetchBinary(stlPath).then((arrayBuffer) => {
          ...
          console.log(actor.getProperty().getColor())
      ...
      createVTKPipeline(
        'vtkContainer2',
        'http://localhost:3000/files/Segmentation_Segment_4.stl',
        0.3
      )
      createVTKCombinePipeline(
        'vtkContainer3',
        'http://localhost:3000/files/Segmentation_Segment_5.stl',
        'http://localhost:3000/files/Segmentation_Segment_4.stl'
      )
