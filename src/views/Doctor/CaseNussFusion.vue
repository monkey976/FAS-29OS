<template>
  <div>
    <div class="input-controls">
      ...
      <button @click="loadNussFile">Load Nuss Model</button>
    </div>

    <div ref="vtkContainer" class="vtk-container"  element-loading-background="rgba(6, 28, 73, 0.2)"/>
    <div ref="vtkContainerNuss" class="vtk-container" element-loading-background="rgba(6, 28, 73, 0.2)" />
...
export default {
  ...
  setup() {
    ...
    const vtkContainerNuss = ref(null)
    const contextNuss = ref(null)
    const modelIdNuss = ref('')
    const representationNuss = ref(2)

    function setRepresentation(rep) {
      representation.value = Number(rep)
      representationNuss.value = Number(rep)
    }
        ...
        .then((data) => {
          ...
          fetch(fileUrl)
            ...
            .then((arrayBuffer) => {
              ...
              vtkReader.parseAsArrayBuffer(arrayBuffer)
              const polyData = vtkReader.getOutputData()
              if (contextNuss.value) {
                const { mapper, renderWindow, renderer } = contextNuss.value
    ...
    function loadFile() {
      ...
      //   return
      // }
      const apiUrl = `https://192.168.32.59:7105/3d_model/preview/${3}`
    ...
    onMounted(() => {
      ...
      if (!contextNuss.value) {
        const fullScreenRenderer = vtkFullScreenRenderWindow.newInstance({
          rootContainer: vtkContainerNuss.value
        })
    ...
    onBeforeUnmount(() => {
      ...
      if (contextNuss.value) {
        const { fullScreenRenderer, actor, mapper } = contextNuss.value
        actor.delete()
        mapper.delete()
        fullScreenRenderer.delete()
        context.value = null
